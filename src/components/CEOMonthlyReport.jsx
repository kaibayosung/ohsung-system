...
import { supabase } from '../supabaseClient'; // 상위 폴더 src/ 에 있는 클라이언트 참조
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
  LineChart, Line, PieChart, Pie, Cell, LabelList // LabelList 추가 (빌드 에러 해결)
} from 'recharts';
...
function CEOMonthlyReport() {
  ...
  const fetchMonthlyData = async () => {
    ...
    const [currSales, currExp] = await Promise.all([
    ...
    setData({
        summary: { totalSales, totalWeight, profit: totalSales - totalExpense },
  ...
  if (loading) return <div style={{padding:'50px', textAlign:'center'}}>월간 경영 분석 중...</div>;

  return (
    <div style={{ padding: '25px', backgroundColor: '#e2e8f0' }}>
      <div style={styles.topGrid}>
        <div style={styles.card}><h3>월간 총 매출</h3><p style={styles.val}>{(data.summary.totalSales/100000000).toFixed(1)}억</p></div>
        <div style={styles.card}><h3>월간 생산량</h3><p style={styles.val}>{Math.round(data.summary.totalWeight/1000)}t</p></div>
        <div style={styles.card}><h3>월간 영업 이익</h3><p style={styles.val}>{(data.summary.profit/100000000).toFixed(1)}억</p></div>
      </div>
      <div style={styles.midGrid}>
        <div style={styles.chartCard}><h4>장비별 실적</h4><div style={{height:250}}><ResponsiveContainer><BarChart layout="vertical" data={data.equipment} margin={{right:80}}><XAxis type="number" hide /><YAxis dataKey="name" type="category" /><Tooltip /><Bar dataKey="sales">{data.equipment.map((e,i)=><Cell key={i} fill={EQ_COLORS[e.name]}/>)}<LabelList dataKey="label" position="right" /></Bar></BarChart></ResponsiveContainer></div></div>
        <div style={styles.chartCard}><h4>주간 매출 추이</h4><div style={{height:250}}><ResponsiveContainer><LineChart data={data.weekly}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name"/><YAxis/><Tooltip/><Line type="monotone" dataKey="sales" stroke="#3182ce" strokeWidth={4}/></LineChart></ResponsiveContainer></div></div>
      </div>
    </div>
  );
}
...
const styles = {
    ...
    card: { backgroundColor:'white', padding:'20px', borderRadius:'12px', textAlign:'center' },
    val: { fontSize:'28px', fontWeight:'900', margin:'10px 0' },
    midGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' },
